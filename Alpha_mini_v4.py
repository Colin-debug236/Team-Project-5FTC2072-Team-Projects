...
async def test_move_robot_right():
    block: MoveRobot = MoveRobot(step=2, direction=MoveRobotDirection.RIGHTWARD)
...
async def test_move_robot_left():
    block: MoveRobot = MoveRobot(step=2, direction=MoveRobotDirection.LEFTWARD)
    (resultType, response) = await block.execute()
    print(f'test_move_robot result:{response}')
    assert resultType == MiniApiResultType.Success, 'test_move_robot timetout'
    assert response is not None and isinstance(response, MoveRobotResponse), 'test_move_robot result unavailable'
...
async def play_tts_welcome():
    block: StartPlayTTS = StartPlayTTS(text="Welcome to test facility. Standby for identification. Please look directly at me.")
    (resultType, response) = await block.execute()
    print(f'{response}')
...
async def play_tts_unauthorised():
...
#--------function for detection if a person is seeking entrance---------------------------------------------------------------------------------

async def Someone_at_the_door():
    ...
    def handler(msg: FaceDetectTaskResponse):
        ...
        if msg.isSuccess and msg.count:
            observer.stop()
            asyncio.create_task(instruction_after_detection(msg.count))

    observer.set_handler(handler)
    observer.start()
    await asyncio.sleep(0)

#--------text to speech with instruction based on successful detection of a person---------------------------------------------------------------

async def instruction_after_detection(count: int):
    if count > 1:
        await StartPlayTTS(text=f' Please stand half a meter in front of me for identification one at a time').execute()
    if count == 1:
        await StartPlayTTS(text=f'Please stand half a meter in front of me for identification').execute()
...
async def main():
    ...
    while True:
        ...
        await Someone_at_the_door()
        await asyncio.sleep(7)
        await play_tts_welcome()
        await facial_recognition()
        await asyncio.sleep(5)  

        if authorised == 1:
            await play_tts_authorised()
